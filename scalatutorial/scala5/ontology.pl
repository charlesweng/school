...
toisaroot(CellPart, [CellPart|RestOfParts]) :-
  is_a(CellPart, RelatedPart),
  toisaroot(RelatedPart,RestOfParts).
...
subconcept(C1, C2) :- part_of(C1, C2).
subconcept(C1, C2) :- is_a(C1, C2).

subconcept(C1, C3) :- part_of(C1, C2), subconcept(C2, C3).
subconcept(C1, C3) :- is_a(C1, C2), subconcept(C2, C3).
