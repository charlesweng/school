...
object Images {
  ...
  def lift1[A,B](f: A => B)(image: Img[A]): Img[B] = p => f(image(p))
  ...
  // of type C.
  def lift2[A,B,C](f: (A,B) => C)(imgA: Img[A])(imgB: Img[B]): Img[C] = p => f(imgA(p), imgB(p))
  ...
  def lift3[A,B,C,D](f: (A,B,C) => D)(imgA: Img[A])(imgB: Img[B])(imgC: Img[C]): Img[D] = p => f(imgA(p),imgB(p),imgC(p))
  ...
  // colour images.
  // def lerpImgClr(imgG: ImgGray)(imgA: ImgClr)(imgB: ImgClr): ImgClr = {
  //   lift3(lerpClr)(imgG)(imgA)(imgB)
  // }
  val lerpImgClr =
    (imgG: ImgGray) => (imgA: ImgClr) => (imgB: ImgClr) => lift3(lerpClr)(imgG)(imgA)(imgB)
  ...
  //   lift2(overlayClr)(imgA)(imgB)
  // }
  val overlayImgClr =
    (imgA: ImgClr) => (imgB: ImgClr) => lift2(overlayClr)(imgA)(imgB)
  ...
  // ImgMask argument.
  // !!FILL ME IN
  // def selectImgClr(imgM: ImgMask)(imgA: ImgClr)(imgB: ImgClr): ImgClr = {
  //   lift3((bool: Boolean, c1: Colour, c2: Colour) => bool match {
  //     case true => c1
  //     case _ => c2
  //   })(imgM)(imgA)(imgB)
  // }
  val selectImgClr =
    (imgM: ImgMask) => (imgA: ImgClr) => (imgB: ImgClr) => {
      lift3((bool: Boolean, c1: Colour, c2: Colour) => bool match {
        case true => c1
        case _ => c2
      })(imgM)(imgA)(imgB)
  ...
  // def msk2clr(imgM: ImgMask): ImgClr = {
  //   lift1((bool: Boolean) => bool match {
  //     case true => ExampleClr.black
  //     case _ => ExampleClr.white
  //   })(imgM)
  // }
  val msk2clr =
    (imgM: ImgMask) => {
  ...
  // def gray2clr(imgG: ImgGray): ImgClr = {
  //   lift1((v: Value) => Colour(v,v,v,v))(imgG)    
  // }
  val gray2clr =
    (imgG: ImgGray) => {
      lift1((v: Value) => Colour(v,v,v,1))(imgG)
    }
}
...
object SpatialTransforms {
  ...
  // y coordinate.
  def translatePt(vec: Vector): TransformPt = p => Point(p.x+vec.x, p.y+vec.y)
  ...
  // coordinate.
  def scalePt(vec: Vector): TransformPt = p => Point(p.x * vec.x, p.y * vec.y)
  ...
  def rotatePt(angle: Double): TransformPt = p => Point(((p.x*cos(angle))-(p.y*sin(angle))),((p.y*cos(angle))+(p.x*sin(angle))))
  ...
  def swirlPt(angle: Double): TransformPt = p => rotatePt( abs(dist(p)) * (2*PI/angle) )(p)

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TranformPt.
  def polarXf(plr: TransformPlr): TransformPt = p => plr2pt(plr(pt2plr(p)))
  ...
  // def translate[A,B](vec: Vector)(f: A => B)(p: Point): Filter = {
  //   f(p) => translatePt(vec.neg)(p)
  // }
  def translate[A](vec: Vector): Filter[A] = {
    (imgA: Img[A]) => (p: Point) => imgA(translatePt(vec.neg)(p))
  }
  ...
  def scale[A](vec: Vector): Filter[A] = {
    (imgA: Img[A]) => (p: Point) => imgA(scalePt(vec.inverse)(p))
  }
  ...
  def rotate[A](angle: Double): Filter[A] = {
    (imgA: Img[A]) => (p: Point) => imgA(rotatePt(-angle)(p))
  }
  ...
  def swirl[A](angle: Double): Filter[A] = {
    (imgA: Img[A]) => (p: Point) => imgA(swirlPt(-angle)(p))
  }
}
...
object Masking {
  ...
  val intersectM =
    (imgA: ImgMask) => (imgB: ImgMask) =>
      lift2((b1: Boolean, b2: Boolean) => (b1 && b2) match {
        case true => true
        case false => false
      })(imgA)(imgB)
  ...
  val xorM =
    (imgA: ImgMask) => (imgB: ImgMask) =>
      lift2((b1: Boolean, b2:Boolean) => ((b1 && !b2) || (!b1 && b2)) match {
        case true => true
        case false => false 
      })(imgA)(imgB)
  ...
  val diffM =
    (X: ImgMask) => (Y: ImgMask) => (p: Point) => intersectM(X)(notM(Y))(p)
}
