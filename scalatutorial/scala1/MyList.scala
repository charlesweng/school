...
sealed trait MyList[A] {
  ...
  // function `f` ofn each element of the old list.  For example:
  //
...
case class MyCons[A](x: A, xs: MyList[A]) extends MyList[A] {
  ...
  def map[B](f: A => B): MyList[B] = {
    foldRight(MyList[B]())((elem,accum) => MyCons(f(elem),accum))
  } 

  def flatMap[B](f: A => MyList[B]): MyList[B] = {
    foldLeft(MyList[B]())((accum,elem) => accum.append(f(elem)))
    // append(foldLeft(MyList[B]())((accum,elem) => f(elem)))
  }

  def filter(pred: A => Boolean): MyList[A] = {
    foldRight(MyList[A]())((elem,accum) => 
      {
        if (pred(elem))
          MyCons(elem,accum)
        else
  ...
  def append(other: MyList[A]): MyList[A] = {
    foldRight(other)((A,B) => MyCons(A,B))
  }
...
case class MyNil[A]() extends MyList[A] {
  ...
  def head: A = throw InvalidOperationException("head throws an exception on an empty list")

  def tail: MyList[A] = throw InvalidOperationException("tail throws an exception on an empty list")

  def init: MyList[A] = throw InvalidOperationException("init throws an exception on an empty list")

  def last: A = throw InvalidOperationException("last throws an exception on an empty list")
