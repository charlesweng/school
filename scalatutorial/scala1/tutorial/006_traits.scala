...
class MyInteger(val value: Int) extends ComparableItem {
  ...
  def compare(first: ComparableItem, second: ComparableItem): Int = compare(first, second) // compare the integers
}
...
class MyIntegerClass(val value: Int) extends ComparableItemTrait with MyNumberTrait {
  def compare(first: ComparableItemTrait, second: ComparableItemTrait): Int = compare(first, second)
}
